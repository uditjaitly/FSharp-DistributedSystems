    ...
    let rec loop n = actor {
        ...
        | StartRouting arr -> 
            for i=1 to numOfReq do
                selfRef <! Route {RouteMessage = [arr.[rand.Next()%numForInit+1];0;0]}
                System.Threading.Thread.Sleep(1000)
            return! loop()
        | Route {RouteMessage=message}->
            if message.[2]= (-1) then
              //  printfn "KEY FOR ROUTE=%i and current source=%i" message.[0] nodeID
      ...
      //                      printfn"Here"
        ...
        //            printfn "DESTINATION REACHED IN HOPS %i" hops
             ...
             //       printfn "ERER for destination=%i" message.[0]
                    count<-count+1
                    avghops<-(float) hops+avghops
                    flag<-1
          ...
          //          printfn "Message of destination %i received by %i to be sent to %i" key nodeID closestNode
           ...
           //             printfn "Message of destination %i received by %i to be sent to %i" key nodeID leafSmaller.MinimumElement
                        ...
                        let nextNode= select pathToActor system
                        ()
      //                  printfn "IN HERE for key=%i!!" message.[0]
         //               printfn "%A" leafs
             //           printfn "%A" nextNode
     ...
     //       printfn "Smaller Leaf Set for %i= %A" nodeID leafSmaller
      ...
      //      printfn "Routing Table for %i= %A" nodeID routingTable
       ...
       //     printfn "NODE ID=%i" nodeID
            ...
            return! loop()
        |PrintInfo p->
  //          printfn "Routing Table of %i = %A" nodeID routingTable
   //         printfn "Greater leaf of %i = %A" nodeID leafGreater
  //          printfn "smaller Leaf f %i = %A" nodeID leafSmaller
            return! loop()
        |_-> 
            counter<-counter+1
            return! loop()
        ...
        actor {
            ...
            | NodeJoin ->
                if itrSecond=1 then
                    ...
                    let selectedActor= select pathToActor system 
               //     printfn "SecondNodeList Element=%i" secondNodeList.[itrSecond]
...
printfn "avghops= %f" (avghops/10000.0)
let totalReq= float numOfNodes * float numOfReq
let threshold= 0.99 * float numOfNodes * float numOfReq

while float count < threshold do
    System.Threading.Thread.Sleep(10)
    if float count >= threshold then
        printfn "Average hops=%f" (avghops/totalReq)
